...
class BananasDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, is_train):
        ...
        print('read' + str(len(self.features)) + (f'training examples'))
...
def read_data_bananas(is_train=True):
    """读取香蕉检测数据集中的图像和标签"""
    data_iter = download_extract('banana-detection')
    csv_fname = os.path.join(data_iter, 'bananas_train' if is_train
    ...
    for img_name, target in csv_data.iterrows():
        images.append(torchvision.io.read_image(
            os.path.join(data_iter, 'bananas_train' if is_train
